...
class DeviceControlWidget(QWidget):
    def __init__(self, device_info, app):
        ...
        self.saturation_factor = 1.4  # Increase saturation by 40%
        self.brightness_factor = 1.2  # Increase brightness by 20%
    ...
    def ambient_loop(self):
        sample_interval = 0.12  # 120 milliseconds
        ...
        while not self.stop_ambient.is_set():
            ...
            try:
                if self.app.alt_key_pressed:
                    ...
                    self.command_queue.put((blended_color, self.current_brightness, time.time()))
                    time.sleep(sample_interval)
                    continue
                ...
                img_small = cv2.resize(cropped_img, (80, 80))  # Reduced size for faster processing

                # Convert image to numpy array
                pixels = img_small.reshape(-1, 3)

                # Define brightness and ratio thresholds
                BRIGHTNESS_THRESHOLD = 200  # Threshold to consider a pixel as bright
                BRIGHT_RATIO_THRESHOLD = 0.2  # 20%
                ...
                bright_ratio = len(bright_pixels) / len(pixels)

                if bright_ratio < BRIGHT_RATIO_THRESHOLD:
                    # If bright pixels are less than 20%, ignore them to avoid reacting to minor whites
                    pixels = pixels[~np.all(pixels >= 240, axis=1)]  # Increase the threshold to 240 to be stricter

                    # Calculate overall brightness after initial filtering
                    overall_brightness = np.mean(pixels)

                    if overall_brightness < 60:
                        pixels = pixels[~np.any(pixels >= 200, axis=1)]
                else:
                    # If bright pixels are significant, do not filter them out
                    # Calculate overall brightness without additional filtering
                    overall_brightness = np.mean(pixels)
                ...
                if len(pixels) == 0:
                    ...
                    # Use a fixed number of clusters for consistency
                    num_clusters = 3
                    ...
                    # Exclude clusters that are near white
                    white_threshold = 240
                    non_white_clusters = [i for i, color in enumerate(colors) if np.max(color) < white_threshold]
                    
                    if non_white_clusters:
                        # Find the largest non-white cluster
                        dominant_cluster = max(non_white_clusters, key=lambda i: counts[i])
                        dominant_color = colors[dominant_cluster].astype(np.uint8)
                    else:
                        # If all clusters are white, use the most common cluster
                ...
                # Send color updates every 240ms
    ...
    def check_and_send_diy_id(self, color_rgb):
        ...
        # Campfire (Orange/Red) -> ID 2
        lower_campfire_hue = 10    # Hue range for orange/red in OpenCV (0-179)
        upper_campfire_hue = 25
    
        # Neon Pink/Violet -> ID 1
        lower_neon_pink_hue = int(290 / 2)  # Convert degrees to OpenCV hue scale
        upper_neon_pink_hue = int(330 / 2)
    
        # Sky (Light Blue) -> ID 3
        lower_sky_blue_hue = int(195 / 2)
        upper_sky_blue_hue = int(225 / 2)
    
        # Check if the color falls within any of the ranges
        if self.is_hue_in_range(h, lower_campfire_hue, upper_campfire_hue):
            diy_id = 2
        elif self.is_hue_in_range(h, lower_neon_pink_hue, upper_neon_pink_hue):
            diy_id = 1
        elif self.is_hue_in_range(h, lower_sky_blue_hue, upper_sky_blue_hue):
            diy_id = 3
    
        if diy_id is not None:
            # Send command to set diy-id
            self.send_command(3, 12, diy_id)

    def is_hue_in_range(self, hue, lower_hue, upper_hue):
        # Handle the case when hue wraps around
        if lower_hue <= upper_hue:
            # Normal range
            return lower_hue <= hue <= upper_hue
        else:
            # Hue wraps around
            return hue >= lower_hue or hue <= upper_hue
    ...
    def enhance_specific_colors(self, color_rgb):
        ...
        # Campfire (Orange/Red)
        lower_campfire = np.array([15, 100, 100])
        upper_campfire = np.array([30, 255, 255])
    
        # Neon lights (Pink/Violet)
        lower_neon = np.array([280, 50, 50])
        upper_neon = np.array([320, 255, 255])
    
        # Sky (Light Blue)
        lower_sky = np.array([170, 50, 50])
        upper_sky = np.array([200, 255, 255])
    
        # Check if the color falls within any of the ranges
        if self.is_color_in_hsv_range(hsv[0][0], lower_campfire, upper_campfire):
            # Enhance campfire colors
            s = np.clip(int(s * 1.5), 0, 255)
            v = np.clip(int(v * 1.2), 0, 255)
            hsv[0][0][1] = s
            hsv[0][0][2] = v
            # Send DIY ID 2 for campfire
            self.send_command(3, 12, 2)
        elif self.is_color_in_hsv_range(hsv[0][0], lower_neon, upper_neon):
            # Enhance neon colors
            s = np.clip(int(s * 1.5), 0, 255)
            v = np.clip(int(v * 1.2), 0, 255)
            hsv[0][0][1] = s
            hsv[0][0][2] = v
            # Send DIY ID 1 for neon
            self.send_command(3, 12, 1)
        elif self.is_color_in_hsv_range(hsv[0][0], lower_sky, upper_sky):
            # Enhance sky colors
            s = np.clip(int(s * 1.5), 0, 255)
            v = np.clip(int(v * 1.2), 0, 255)
            hsv[0][0][1] = s
            hsv[0][0][2] = v
            # Send DIY ID 3 for sky
            self.send_command(3, 12, 3)
        else:
            # For other colors, enhance saturation and brightness slightly
            s = np.clip(int(s * self.saturation_factor), 0, 255)
            v = np.clip(int(v * self.brightness_factor), 0, 255)
            hsv[0][0][1] = s
            hsv[0][0][2] = v
    ...
    def is_color_in_hsv_range(self, hsv_color, lower_hsv, upper_hsv):
        h, s, v = hsv_color
        if lower_hsv[0] <= h <= upper_hsv[0] and lower_hsv[1] <= s <= upper_hsv[1] and lower_hsv[2] <= v <= upper_hsv[2]:
            return True
        return False
    ...
    def command_sender_loop(self):
        ...
        command_timeout = 1.2  # Commands older than 2 seconds are discarded
        ...
        while True:
            try:
                ...
                if (hasattr(self, 'fade_transition_start_time') and self.fade_transition_start_time is not None) \
                    ...
                    # Enhance specific colors and adjust saturation/brightness
                    enhanced_color = self.enhance_specific_colors(avg_color)
    
                    # Convert enhanced_color to integer value
                ...
                print(f"Error in command_sender_loop for {self.name}: {e}")
