...
class DeviceControlWidget(QWidget):
    ...
    def flash_selected_region(self):
        # Capture the screen to get full dimensions
        img = ImageGrab.grab(all_screens=True)
        img_np = np.array(img)

        # Get the full image dimensions
        full_width = img_np.shape[1]
        full_height = img_np.shape[0]

        # Apply margins
        left = int(self.region["margin_left"] / 100 * full_width)
        right = int(full_width - self.region["margin_right"] / 100 * full_width)
        top = int(self.region["margin_top"] / 100 * full_height)
        bottom = int(full_height - self.region["margin_bottom"] / 100 * full_height)
    ...
    def ambient_loop(self):
        ...
        while not self.stop_ambient.is_set():
            ...
            try:
                ...
                desired_width = 80
                desired_height = max(int(desired_width / aspect_ratio), 1)  # Prevent division by zero
                img_small = cv2.resize(cropped_img, (desired_width, desired_height))
