...
class DeviceControlApp(QMainWindow):
    def __init__(self):
        ...
        self.focus_keywords = ["discord", "slack", "notepad", "reddit", "code", "chatgpt", "9gag"]
    ...
    def ambient_loop(self):
        sample_interval = 0.12  # 80 milliseconds
        color_transition_interval = 0.36  # 160 milliseconds
        sample_buffer = []
        ...
        while not self.stop_ambient.is_set():
            ...
            try:
                ...
                # Adjusted threshold to allow bright colors
                threshold = 246
                pixels = pixels[~np.all(pixels >= threshold, axis=1)]
    
                # Removed additional filtering for dark scenes
    ...
    def enhance_specific_colors(self, color_rgb):
        ...
        if self.is_color_in_range(h, lower_green, upper_green) or \
           ...
           self.is_color_in_range(h, lower_neon, upper_neon):
            ...
            s = np.clip(int(s * 1.6), 0, 255)
            v = np.clip(int(v * 1.4), 0, 255)
            hsv[0][0][1] = s
            hsv[0][0][2] = v
            color_array = cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB)
            return color_array[0][0]
