...
class DeviceControlApp(QMainWindow):
    ...
    def ambient_loop(self):
        while not self.stop_ambient.is_set():
            ...
            try:
                ...
                # If the screen is too dark, set color to black
                if overall_brightness < 36:
                ...
                # Send the command
                self.command_queue.put((blended_color, start_time))
    ...
    def increase_saturation(self, color_rgb):
        ...
        h, s, v = hsv[0][0]
        s = np.clip(int(s * 1.5), 0, 255)  # Increase saturation by 50%
        hsv[0][0][1] = s
