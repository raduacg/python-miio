...
class DeviceControlWidget(QWidget):
    def __init__(self, device_info, app):
        ...
        self.previous_detection = None  # To track the last detected object
    ...
    def enhance_specific_colors(self, color_rgb):
        ...
        # Sky (Added)
        lower_sky = np.array([90, 50, 70])
        upper_sky = np.array([130, 255, 255])

        detected_object = None

        # Detection Logic
        if self.is_color_in_range(h, lower_neon, upper_neon):
            detected_object = 'neons'
        elif self.is_color_in_range(h, lower_orange, upper_orange):
            detected_object = 'campfire'
        elif self.is_color_in_range(h, lower_sky, upper_sky):
            detected_object = 'sky'

        # Send Commands Based on Detection
        if detected_object and detected_object != self.previous_detection:
            if detected_object == 'neons':
                self.send_command(3, 12, 1)
                print("Neons detected: Sent command (3, 12, 1)")
            elif detected_object == 'campfire':
                self.send_command(3, 12, 2)
                print("Campfire detected: Sent command (3, 12, 2)")
            elif detected_object == 'sky':
                self.send_command(3, 12, 3)
                print("Sky detected: Sent command (3, 12, 3)")
            self.previous_detection = detected_object
            return color_rgb  # Return original color without enhancing
        else:
            # Existing Enhancement Logic for Other Objects (e.g., Trees)
            if self.is_color_in_range(h, lower_green, upper_green):
                ...
                return enhanced_color[0][0]
            else:
                return color_rgb  # Return original color if no enhancement is needed
